...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'GXScheduler'">
        <xsl:call-template name="RenderGXScheduler"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- GXScheduler design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderGXScheduler">
				...
				<TR>
					<TD>            
						<xsl:attribute name="background">
							...
							<xsl:text>\GXScheduler.PNG</xsl:text>
						</xsl:attribute>
					</TD>
				</TR>
     	</table>   
